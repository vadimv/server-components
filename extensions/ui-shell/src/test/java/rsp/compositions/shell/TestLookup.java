...
public class TestLookup implements Lookup {
    ...
    private final Map<ContextKey<?>, List<BiConsumer<?, ?>>> watchers = new HashMap<>();
    ...
    @Override
    public <T> Registration watch(final ContextKey<T> key, final BiConsumer<T, T> handler) {
        watchers.computeIfAbsent(key, k -> new ArrayList<>()).add(handler);
        return () -> {
            final List<BiConsumer<?, ?>> keyWatchers = watchers.get(key);
            if (keyWatchers != null) {
                keyWatchers.remove(handler);
            }
        };
    }
