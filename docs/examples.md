...
- Demonstrates the end-to-end `compositions` stack:
  ...
  - **Auth** — a separate `Composition` for `/auth/login` using `SimpleAuthProvider` + `LoginBlock`; the `ui-http-auth` adapter redirects anonymous initial requests before rendering.
