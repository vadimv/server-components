...
- URL: <http://localhost:8080/forms/name>
- Demonstrates: exact GET and POST page routes alongside a generic
  `/api/health` route in one UI-facing `Router`, path-scoped page rendering, sealed
...
- Demonstrates the end-to-end `compositions` stack:
  - **Routing** — nested groups declare `/posts`, `/posts/new`, `/posts/{id}`, `/comments`, and `/comments/{id}` beside their block factories; `Composition` derives and validates the immutable `RouteTable<BlockTarget>`.
