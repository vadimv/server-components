 ...
 */
public final class RemotePageCommandEncoder implements Function<PageCommand, String> {
    ...
    @Override
    public String apply(PageCommand pageCommand) {
        if (pageCommand instanceof PageCommand.SetRenderNum renderNumCommand) {
            return setRenderNum(renderNumCommand.renderNum());
        } else if (pageCommand instanceof PageCommand.ListenEvent listenEventCommand) {
            return listenEvents(listenEventCommand.events());
        } else if (pageCommand instanceof PageCommand.ForgetEvent forgetEventCommand) {
            return forgetEvent(forgetEventCommand.eventType(), forgetEventCommand.elementPath());
        } else if (pageCommand instanceof PageCommand.ExtractProperty extractPropertyCommand) {
            return extractProperty(extractPropertyCommand.descriptor(), extractPropertyCommand.path(), extractPropertyCommand.name());
        } else if (pageCommand instanceof PageCommand.ModifyDom modifyDomCommand) {
            return modifyDom(modifyDomCommand.domChanges());
        } else if (pageCommand instanceof PageCommand.PushHistory pushHistoryCommand) {
            pushHistory(pushHistoryCommand.path());
        } else if (pageCommand instanceof PageCommand.SetHref setHrefCommand) {
            return setHref(setHrefCommand.path());
        }
    ...
    public String setRenderNum(final int renderNum) {
        return addSquareBrackets(joinString(SET_RENDER_NUM, renderNum));
    }

    public String listenEvents(final List<Event> events) {
    ...
    public String forgetEvent(final String eventType, final VirtualDomPath path) {
    ...
    public String extractProperty(final int descriptor, final VirtualDomPath path, final String name) {
    ...
    public String modifyDom(final List<DomChange> domChanges) {
    ...
    public String evalJs(final int descriptor, final String js) {
