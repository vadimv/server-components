...
class PathPatternTests {
    ...
    @Test
    void should_match_empty_path_for_empty_pattern() {
        assertTrue(PathPattern.of("").match(Path.of("")));
    }

    @Test
    void should_not_match_non_empty_path_for_empty_pattern() {
        assertFalse(PathPattern.of("").match(Path.of("/1")));
    }

    @Test
    void should_not_match_non_empty_path_for_non_empty_pattern() {
        assertFalse(PathPattern.of("/1").match(Path.of("")));
    }

    @Test
    void should_match_path_for_pattern() {
        assertTrue(PathPattern.of("/1/2").match(Path.of("/1/2")));
    }

    @Test
    void should_not_match_for_path_shorter_with_slash() {
        assertFalse(PathPattern.of("/1/2").match(Path.of("/1/")));
    }

    @Test
    void should_not_match_for_path_shorter_than_pattern() {
        assertFalse(PathPattern.of("/1/2").match(Path.of("/1")));
    }

    @Test
    void should_match_path_for_pattern_with_wildcard() {
    ...
    @Test
    void should_not_match_path_for_pattern_with_wildcard() {
        assertFalse(PathPattern.of("/1/2/*").match(Path.of("/1/")));
    }

    @Test
    void should_match_path_for_pattern_with_params() {
        assertTrue(PathPattern.of("/:p1/2/:p2").match(Path.of("/1/2/3")));
    }

    @Test
    void should_not_match_path_for_pattern_with_params() {
        assertFalse(PathPattern.of("/:p1/1/").match(Path.of("/1/2/3")));
    }

    @Test
    void should_match_path_for_pattern_with_params_and_wildcards() {
        assertTrue(PathPattern.of("/:p1/2/:p2/*").match(Path.of("/1/2/3/4/5")));
    }

    @Test
    void should_match_path_for_pattern_with_params_and_regex() {
