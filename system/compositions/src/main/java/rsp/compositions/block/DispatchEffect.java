...
/**
 * Declarative effect that dispatching a {@link BlockAction} has on the
 * scene's routed block. Read by the agent runtime to decide whether to
...
public enum DispatchEffect {
    ...
    /**
     * Dispatch does not change the routed block. No wait between steps.
     * Examples: {@code page}, {@code select_all}, {@code set_field}, {@code delete}
     * on a list (data-only refresh — same routed block).
     */
    NONE,

    /**
     * Dispatch will trigger a scene rebuild — typically opens or closes a
     * block, changing the routed block class. The runtime arms a
