 ...
 *
 * <pre>{@code
 * // ClassKey example - key identity is UserService.class
 * ContextKey<UserService> usersKey = new ContextKey.ClassKey<>(UserService.class);
 * UserService users = context.get(usersKey);  // No cast needed!
 *
...
public sealed interface ContextKey<T>
     ...
     * <pre>{@code
     * // Creating a ClassKey
     * ContextKey<UserService> key = new ContextKey.ClassKey<>(UserService.class);
     *
     * // Setting value in context
     * context.with(key, routerInstance);
     *
     * // Getting value from context (type-safe, no cast needed)
     * UserService users = context.get(key);
     *
     * // Convenience shorthand (bypasses explicit ClassKey creation)
     * context.with(UserService.class, usersInstance);
     * UserService users = context.get(UserService.class);
     * }</pre>
     *
     * @param <T> the type of value stored under this key
